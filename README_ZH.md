...
`/bg`（别名 `/background`）把当前会话移入后台、继续运行，终端切到新会话，并打开会话管理界面（与 `/agentview` 同一个）。空输入按 `←` 等同；后台会话等你输入时底栏显示「← N agents」，`Esc` 回到刚转后台的会话（在界面里选过别的会话则以选择为准）。

- 后台会话跑在本进程内，回合 / 工具 / 审批照常；TUI 退出即停止，日志保留，`/resume` 或会话界面都能找回，没有 supervisor 进程。
- 会话界面按工作区列出会话，每行带实时状态（等待输入 · 运行中 · 已完成 ……）和一行来自会话自身输出的摘要，不额外调用模型；`Enter` 进入某行，`Ctrl+X` 停止光标所在的后台会话（当前附着的停不了）。
- 需要审批的后台会话显示为**等待输入**；审批面板标注它来自哪个会话。
...
- **后台会话活在本进程内**：TUI 退出即停止；状态与行摘要来自会话自身输出、无额外摘要模型调用；worktree 隔离与 shell 后台任务尚未提供。
