...
| 字段 | 默认/来源 | 说明 |
| --- | --- | --- |
| `remoteEndpoint` | 登录文件中的地址 | 云端账号地址；同时用于 `/connect` 与本地账号模型 |
| `remoteModelTimeoutMs` | `120000` | 云端推理超时，1000–600000 毫秒 |
| `remoteModelContextWindow` | `262144` | 本地上下文预算，1024–2097152 tokens |
| `remoteModelMaxTokens` | `32768` | 单次回复预算，1–262144 tokens |
...
| `cwd` | 启动目录所在的 git worktree 根（不在任何 worktree 内时为 `process.cwd()`；家目录的 dotfiles 仓不算） | TUI 会话侧工作区：agent meta、`@` 补全/提及展开、/resume 过滤、状态栏；恢复已有会话时以该会话持久化的 cwd 为准。在验证的 DSH 0.1.5-rc.1 上，sandbox-policy 优先采用会话 header.cwd，组合层 root 仅为缺省回退；自定义 fs/Shell provider 仍须检查其配置 |
